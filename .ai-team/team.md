# GIFBot — Squad Roster

## Project Context

| Field | Value |
|-------|-------|
| **Project** | GIFBot |
| **Description** | Interactive Twitch bot — ASP.NET backend with SignalR, Blazor frontend |
| **Stack** | C#, .NET, Blazor, SignalR, MudBlazor (target) |
| **User** | Georgia Nelson (gnelson@microsoft.com) |
| **Goal** | Modernize: .NET 8 → .NET 10, Blazor WASM → Blazor Server, Telerik → MudBlazor, UI refresh |

## Team

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Mark | Lead | .ai-team/agents/mark/charter.md | ✅ Active |
| Helly | Frontend Dev | .ai-team/agents/helly/charter.md | ✅ Active |
| Irving | Backend Dev | .ai-team/agents/irving/charter.md | ✅ Active |
| Dylan | Tester | .ai-team/agents/dylan/charter.md | ✅ Active |
| Milchick | TPM | .ai-team/agents/milchick/charter.md | ✅ Active |
| Scribe | Scribe | .ai-team/agents/scribe/charter.md | 📋 Silent |
| Ralph | Work Monitor | — | 🔄 Monitor |
